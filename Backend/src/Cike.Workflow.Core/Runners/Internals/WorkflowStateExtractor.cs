...
public class WorkflowStateExtractor(ILogger<WorkflowStateExtractor> logger) : IWorkflowStateExtractor
{
    ...
    public WorkflowState Extract(WorkflowExecutionContext workflowExecutionContext)
    {
        var state = new WorkflowState
        {
            ...
            DefinitionId = workflowExecutionContext.WorkflowGraph.Workflow.DefinitionInfo.DefinitionId,
            DefinitionVersionId = workflowExecutionContext.WorkflowGraph.Workflow.DefinitionInfo.Id,
            DefinitionVersion = workflowExecutionContext.WorkflowGraph.Workflow.DefinitionInfo.Version,
            CorrelationId = workflowExecutionContext.CorrelationId,
            Name = workflowExecutionContext.WorkflowGraph.Workflow.DefinitionInfo.Name,
    ...
    private async Task ApplyActivityExecutionContextsAsync(WorkflowState state, WorkflowExecutionContext workflowExecutionContext)
    {
        ...
        foreach (var contextState in state.ActivityExecutionContexts.Where(x => x.ParentContextId.HasValue))
        {
            var parentContextId = contextState.ParentContextId;
            if (parentContextId == null || !lookup.TryGetValue(parentContextId.Value, out var parentContext))
            {
    ...
    private void LogSkippedStateReference(
        ...
        string skipKind,
        long? activityExecutionContextId = null,
        string? scheduledActivityNodeId = null,
        long? completionCallbackOwnerInstanceId = null,
        string? completionCallbackChildNodeId = null)
    {
        var targetIdentity = workflowExecutionContext.WorkflowGraph.Workflow.DefinitionInfo;
        var isWorkflowDefinitionVersionMigration = state.DefinitionVersionId != targetIdentity.Id;
