namespace Cike.Workflow.Core.Models;

/// <summary>
/// Represents a list of outcomes that can be send when completing an activity. This information is used by <see cref="Activities.FlowchartActivity.Flowchart"/>.
/// </summary>
/// <param name="Names">A list of outcome names.</param>
public record Outcomes(params string[] Names)
{
    public static readonly Outcomes Default = new(null!, "Done");
    public static readonly Outcomes Empty = new();
}
