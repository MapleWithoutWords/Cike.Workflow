...
internal class PropertyBasedActivityPortResolver(ActivityPortRegistry activityPortRegistry) : IActivityPortResolver, ISingletonDependency
{
    /// <inheritdoc />
    ...
    /// <inheritdoc />
    public async ValueTask<IEnumerable<ActivityPort>> GetActivityPortsAsync(IActivity activity, CancellationToken cancellationToken = default)
    {
        return await activityPortRegistry.GetActivityPortsAsync(activity, cancellationToken);
    }
