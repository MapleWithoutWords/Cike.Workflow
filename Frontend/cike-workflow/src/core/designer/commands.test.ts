...
import { getCanvasState, getNodePosition, setCanvasState } from "./metadata";
...
describe("MoveNodeCommand", () => {
  ...
  it("Move_FromNullPosition_UndoPreservesCanvasViewport", () => {
    const activity = new Flowchart();
    setCanvasState(activity, { zoom: 1.5, panX: 20, panY: 30 });
    const command = makeMoveNodeCommand(activity, null, { x: 5, y: 6 });
    command.apply();
    command.undo();
    expect(getNodePosition(activity)).toBeNull();
    expect(getCanvasState(activity)).toEqual({ zoom: 1.5, panX: 20, panY: 30 });
  });
});
