...
将 7 个带附加行为的 Store 迁移为框架自定义仓储、纯 CRUD 的 BookmarkQueueItem 交框架默认仓储、删除自建 BaseStore 体系，最后统一落地仓储测试。来源 spec：同目录 `PRD.md`。
...
- [ ] `IWorkspaceRepository` / `IFolderRepository` 定义在 Domain 层，继承框架 `IRepository<TEntity, long>`
- [ ] 实现类继承框架 EF Core 仓储基类、标注 Scoped 约定注册，天然覆盖同实体默认仓储
- [ ] 缓存同步：插入 / 更新后缓存被 set，删除后缓存被 remove（单实体 + 批量路径全覆盖）
- [ ] Handler 机械替换完成，业务逻辑无变化；列表 / 详情 / 增删改端点行为与迁移前一致
- [ ] 旧 Store 与接口删除，解决方案编译零残留引用
- [ ] 现有 API 测试全部通过
...
- [ ] 4 个仓储接口定义在 Domain 层，继承框架 `IRepository<TEntity, long>`；实现类约定注册覆盖默认仓储
- [ ] 插入路径序列化：影子属性列写入预期 JSON（含 Bookmark / 执行记录的空值写 null 语义）
- [ ] **Update 路径序列化补齐：更新后影子属性列写入新值 JSON，4 个仓储一致覆盖**
- [ ] 读路径与现状对齐：单查反序列化；Bookmark 列表查询反序列化；其余列表不反序列化
- [ ] 损坏 JSON 单查时记日志回退默认值、不抛异常
- [ ] WorkflowDefinition / Bookmark 保留排序重载且默认排序生效；投影重载未保留
- [ ] Handler 机械替换完成，业务逻辑无变化；定义相关端点行为与迁移前一致
- [ ] 4 个旧 Store 与接口删除，解决方案编译零残留引用
- [ ] 现有 API 测试全部通过
...
- [ ] `BookmarkQueueItemStore` / `IBookmarkQueueItemStore` 删除，该实体经框架默认仓储可用（编译期可注入）
- [ ] `BaseStore` / `IBaseStore` 删除，全解决方案 grep 零残留
- [ ] 全解决方案编译通过、现有 API 测试全部通过
...
- [ ] 测试基座：模块加载与生产一致，SQLite in-memory 建表成功（含 json 影子属性列的方言兼容处理）
- [ ] 缓存服务内存替身可注入，测试不依赖外部 MySQL / Redis
- [ ] DI 覆盖解析：7 个自定义仓储与同实体 `IRepository` / `IReadOnlyRepository` 解析到同一实现类型
- [ ] 默认仓储解析：BookmarkQueueItem 解析到框架默认 EF Core 仓储（未被误覆盖）
- [ ] CRUD 行为：插入返回已填充主键、按 id 删除幂等、分页 Total / Items 正确
- [ ] 序列化 round-trip：插入 / **更新（修复项断言）** / 单查还原（Bookmark 含列表查询）全绿
- [ ] 缓存同步：Workspace / Folder 写后 set、删后 remove 可观察
- [ ] 排序重载：指定排序生效、默认 `CreatedAt desc` 生效
- [ ] docs/testing.md 第 7 节更新为 SQLite in-memory 方案，数据库类测试标注 Integration 分类
- [ ] `dotnet test` 全量通过
